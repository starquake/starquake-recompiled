...
Yes: the energy bar only ever goes down in one place, `Game::reduce_bar`
(`games/starquake/src/hud.rs:138`), and the time drain is its caller at
`games/starquake/src/blob.rs:565`. That is why training mode (#4) is one
condition rather than a rework.
```
