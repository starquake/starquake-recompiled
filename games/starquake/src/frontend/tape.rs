...
pub fn read(path: &Path) -> Result<(Vec<u8>, Option<Vec<u8>>), String> {
    let is_tap = path
        .extension()
        .is_some_and(|e| e.eq_ignore_ascii_case("tap"));
    if !is_tap && !is_zip(path) {
        return starquake::assets::read_game(path);
    }
    let tape = load(path, starquake::assets::is_supported_tape)
        .map_err(|why| format!("{}: {why}", path.display()))?;
    starquake::assets::read_tape(&tape.bytes)
}
...
pub fn not_found_message(folders: &[PathBuf]) -> String {
    let mut msg = String::from(
        "error: no copy of Starquake found.\n\n\
         This program contains no part of the original game and reads its graphics, maps,\n\
         text and sound from your own copy at startup. Put the tape, or the zip it was\n\
         downloaded in, next to the program, or name it on the command line:\n\n    \
